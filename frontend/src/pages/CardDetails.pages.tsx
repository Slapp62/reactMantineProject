      ...
      <Card shadow="sm" padding="lg" radius="md" withBorder mx="auto">
        ...
        <Card.Section p={15}>
          <Flex direction="column" gap={10}>
            <Text size="xl" fw={500}>
              <strong>Title:</strong> {translatedText ? translatedText[0] : card?.title}
            </Text>
            <Text size="md">
              <strong>Subtitle:</strong> {translatedText ? translatedText[1] : card?.subtitle}
            </Text>
            <Text size="md" w="95%">
          ...
          <Flex justify="space-between" mt={10} gap={10} direction="column">
            <List spacing={5} style={{ wordBreak: 'break-word' }} w="100%">
              ...
              {card?.web && (
                <ListItem>
                  <strong>Website:</strong> {card?.web}
                </ListItem>
              )}
              <hr />
            </List>
            ...
            <Flex justify="space-between" direction="column" gap={20}>
              <List spacing={5} style={{ wordBreak: 'break-word' }}>
                ...
                <ListItem>
                  <strong>Country:</strong> {card?.address.country}
                </ListItem>
                <ListItem>
                  <strong>City:</strong> {card?.address.city}
                </ListItem>
                {card?.address.state && (
                  <ListItem>
                    <strong>State:</strong> {card?.address.state}
                  </ListItem>
                )}
                <ListItem>
                  <strong>Street:</strong> {card?.address.street}
                </ListItem>
                <ListItem>
                  <strong>Number:</strong> {card?.address.houseNumber}
                </ListItem>
                <ListItem>
                  <strong>Zipcode:</strong> {card?.address.zip}
                </ListItem>
              </List>
